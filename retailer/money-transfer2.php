...
$(document).ready(function(){
					...
					success	: function(data) {	
						...
						if(obj["RESP_CODE"]=='200') {
							...
							function() {
								...
								$("#customerBalance").val(obj["DATA"]["SENDER_AVAILBAL"]);
								$.post("yesbank/list-beneficiary.php",{js_data: obj, ajax: 'true'}, function(jd){
						...
						if(obj["RESP_CODE"]=='200') {	
						  ...
						  //  	$('#otpRequestFor').val('CUSTVERIFICATION');
                				...
                				$.ajax({ 
                					url		: "dmt-yesbank.php",
                					type	: "GET",
                					data	: "request=otpGenerate&mobile="+mobile+"&request_for=CUSTVERIFICATION",
                					async : false,
                					success	: function(data) {
                						//console.log(data);
                						var obj = $.parseJSON(data);
	...
	$("#submitOtpVerification").click(function() {
		...
		if(mobile=="" ||  otpRequestFor=="" || otpRefCode=="" || otpCode=="") {
	...
	$("#submitTransactionHistroy").click(function() {
		...
		if(mobile=="") {
			...
			$.ajax({ 
				...
				// 			});
				// 		});
				            html+='<div class="dmt box">';
                            html+='<div class="box-heading">';
                            html+='<div class="panel-title pull-left"><i class="fa fa-list-alt"></i> List Transaction</div>';
                            html+='<div class="pull-right">';
                            html+='<a href="javascript:void(0)" onclick="getBackBeneList()" class="btn btn-sm btn-success">Back</a>';
                            html+='</div>	';
                            html+='</div>';
                            ...
                            html+='<tbody>';
                            var jsList = obj;
                            var jsTransaction = jsList['DATA']['TRANSACTION_DETAILS'];
                            for (key in jsTransaction) { 
                                  var trn=jsTransaction[key];
                            html+='<tr>';
                            html+='<td align="center">';
                            html+='<a href="javascript:void(0);"></a>';
                            html+='</td>';
                            html+='<td>'+ trn.TRANSACTION_DATE +'</td>';
                            html+='<td>'+trn.BENE_NAME+'</td>';
                            html+='<td>'+trn.BENE_ID+'</td>';
                            html+='<td>'+trn.BANK_ACCOUNTNO+'</td>';
                            html+='<td>'+trn.BANK_REFERENCE_NO+'</td>';
                            html+='<td>'+trn.TRANSFER_AMOUNT+'</td>';
                            html+='<td>'+trn.TRANSACTION_STATUS+'</td>';
                            html+='<td>';
                             if(trn.TRANSACTION_STATUS=='FAILED' || trn.TRANSACTION_STATUS=='FALSE') {
                            html+='<a href="javascript:void(0)" onClick="getRefund('+trn.CUSTOMER_REFERENCE_NO+','+trn.TRANSFER_AMOUNT+');" class="btn btn-xs btn-primary">Refund</a>';
                            } 
                            html+='</td>';
                            html+='</tr>'; 
                            } 
                            ...
                            html+='</div>';
                            $("#boxSenderTransaction").show();	
                            $("#boxSenderTransaction").html(html);
                           swal({ title: "SUCCESS", text: obj["RESP_MSG"], type: "success"}); 
					} else if (obj["RESP_CODE"]=='224') {
						swal({ title: "Error", text: obj["RESP_MSG"], type: "error"});
					} else {
	...
	$("#submitMoneyRemittance").click(function() {
		...
		if(mobile=="" || benId=="" || benMobile=="" || benName=="" || benAccount=="" || benBank=="" || benIfsc=="" || benType=="" || benAmount=="" || kycStatus=="") {
			...
			if(parseInt(mobile.length)!='10') {
				...
				var benLimit = $('#customerBalance').val().trim();
				if(benAmount > benLimit) {
					swal({ title: "Error", text: "You sender limit is not available", type: "error"});
					return false;
				} else {
					swal({ title: "Processing", text: "Please wait a moment..", imageUrl: "../images/preloader.gif", showConfirmButton: false});
	...
	//Resend OTP
	$("#submitRefundAmount").click(function() {
		var mobile = $('#mobile').val().trim();
		var refundTxnId = $('#refundTxnId').val().trim();
		var refundRefCode = $('#refundReferenceCode').val().trim();
		var refundOtp = $('#refundOtp').val().trim();
		var refundAmount = $('#refundAmount').val().trim();
		if(mobile=="" || refundTxnId=="" || refundRefCode=="" || refundOtp=="") {
			swal({ title: "Error", text: "Mobile/OTP reference number cannot be blank", type: "error"});
			return false;
		} else {
			if(parseInt(mobile.length)!='10') {
				...
				$.ajax({ 
					url		: "dmt-yesbank.php",
					type	: "GET",
					data	: "request=transactionRefund&mobile="+mobile+"&txn_id="+refundTxnId+"&otp_ref_code="+refundRefCode+"&otp="+refundOtp+"&amount="+refundAmount,
					async : false,
					success	: function(data) {
						//console.log(data);
						var obj = $.parseJSON(data);
...
function getRefund(id,amount) {
	var rCode = generateOtps('9','CUSTREFUND','0');	
	if(rCode!='') {
		var mobile = $('#mobile').val().trim();
		if(mobile=="") {
			swal({ title: "Error", text: "Mobile or Referece Code or OTP cannot be blank", type: "error"});
		...
		<div class="row">
			...
			<div class="col-sm-12 min-height-480">
				...
				<div style="display:none;">
					...
					<input type="hidden" name="customer_balance" id="customerBalance"/>
				</div>
