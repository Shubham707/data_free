...
$nFrom = date("Y-m-d 00:00:00", strtotime('-168 HOUR'));
...
$sWhere = "WHERE recharge.uid = '".$_SESSION['apiuser_uid']."' AND recharge.request_date LIKE '%".$aFrom."%' ";

$array['recharge'] = getRechargeStatusList();
$statement = "apps_recharge recharge LEFT JOIN operators opr ON recharge.operator_id = opr.operator_id $sWhere ORDER BY recharge.request_date ASC";

$query = $db->query("SELECT recharge.*, opr.operator_name FROM {$statement} ");
