		...
		if($user_api_info->ip1==$ip || $user_api_info->ip2==$ip || $user_api_info->ip3==$ip || $user_api_info->ip4==$ip) {
			...
			if($agent_info) {
				if(isset($_GET['request']) && $_GET['request']=='senderValidate') {
					...
					include(DIR."/ezypay/beneficiary-validate.php");
					echo $jsDmtResponse;
					exit();
					...
					include(DIR."/ezypay/money-remittance.php");
			...
			echo '{"EZY_RSP":"23","EZY_MSG":"Invalid request IP '.$ip.'"}';		
			exit();
		}
	} else {
		echo '{"EZY_RSP":"24","EZY_MSG":"Invalid API Key or User ID"}';		
		exit();
	}
} else {
	echo '{"EZY_RSP":"25","EZY_MSG":"Parameters are missing"}';		
	exit();
}
