...
$db->query("INSERT INTO apps_reverse_response (api_id, reverse_response_content, response_time) VALUES ('9', '".$content."', NOW())");
		...
		if($agent_info) {
			...
			if($api_status=='SUCCESS') {
				...
				if($offline_retailer)
							...
							$db->execute("UPDATE apps_recharge SET status='8',api_id='11',org_api_id='9', status_details='Transaction Submitted', request_txn_no='".$request_txn_no."', api_txn_no='".mysql_real_escape_string($api_txn_no)."', api_status='".mysql_real_escape_string($api_status)."' WHERE recharge_id='".$recharge_id."' ");
