...
$isMasterRecharge = $db->queryUniqueObject("SELECT is_recharge FROM apps_user WHERE uid='".$agent_info->mdist_id."' AND is_recharge='a' ");
$isDistRecharge = $db->queryUniqueObject("SELECT is_recharge FROM apps_user WHERE uid='".$agent_info->dist_id."' AND is_recharge='a' ");
if($isMasterRecharge && $isDistRecharge) {
	//
					...
					if($operator_info->status == '1') {
						if($amount >= $operator_info->minimum_amount && $amount <= $operator_info->maximum_amount) {			
							...
							if($isfund && (($isfund->balance - $isfund->cuttoff) >= ($amount + $sCommission['samount']))) {
								$status = '1';
								$db->execute("INSERT INTO `apps_recharge`(`uid`, `recharge_mode`, `api_id`, `service_type`, `operator_id`,`org_operator_id`, `account_no`, `amount`, `surcharge`, `status`, `status_details`, `request_date`, `reference_txn_no`, `recharge_ip`) VALUES ('".$agent_info->uid."', '".$mode."', '".$api_id."', '".$operator_info->service_name."', '".$operator_info->operator_id."','".$operator_info->operator_id."', '".$account."', '".$amount."', '".$sCommission['samount']."', '".$status."', '', NOW(), '".$reference_txn_no."', '".$ip."')");
								...
								if($operator_info->service_type == '5') {
									...
									$db->execute("INSERT INTO `orders`(`order_date`, `recharge_id`, `agent_uid`, `operator_id`, `product_id`, `order_amount`, `customer_name`, `customer_mobile`, `customer_email`, `customer_address`, `customer_city`, `customer_pincode`, `customer_state` , `order_status`) VALUES (NOW(), '".$recharge_id."', '".$agent_info->uid."', '".$operator_info->operator_id."', '".$product_id."', '".$amount."', '".$customer_name."', '".$customer_mobile."', '".$customer_email."', '".$customer_address."', '".$customer_city."', '".$customer_pincode."', '".$customer_state."', 'pending')");
								}
									...
									if($commit) {
										...
										*/
										if($api_id == '1') {
											...
											include(DIR . "/library/clickecharge-recharge-api.php");
										} else if($api_id == '6') {
											...
											include(DIR . "/library/ezypay-recharge-api.php");
										} else if($api_id == '14') {
										...
										if($status == '0') {
											...
											*/
											if($agent_info->user_type!='1') {
												...
												$jio = $db->queryUniqueObject("SELECT * from offline_denominations where service_type_id='".$operator_info->service_type."' and user_type=5 and id!=1 and status=1 and (FIND_IN_SET ($amount, amount_values) OR $amount between amount_from and amount_to) ");
												$offline=$db->queryUniqueObject("SELECT * from offline_denominations where service_type_id='".$operator_info->service_type."' and user_type=5 and status=1 and id!=1 and (FIND_IN_SET ($amount, amount_values) OR $amount between amount_from and amount_to)");
												...
												*/
												...
												//Error Code: Recharge Failed	
												$utility_status = $db->queryUniqueValue("SELECT status from offline_denominations where id=1 and service_type_id=0 ");
												$jio = $db->queryUniqueObject("SELECT * from offline_denominations where service_type_id='".$operator_info->service_type."' and user_type=5 and status=1 and (FIND_IN_SET ($amount, amount_values) OR $amount between amount_from and amount_to) ");
												$offline=$db->queryUniqueObject("SELECT * from offline_denominations where service_type_id='".$operator_info->service_type."' and user_type=5 and status=1 and id!=1 and (FIND_IN_SET ($amount, amount_values) OR $amount between amount_from and amount_to)");
												$jio_from=$jio->amount_from;
												$jio_to=$jio->amount_to;
