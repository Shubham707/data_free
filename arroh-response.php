<?php
/*
 * Last update by Sunil on 21-12-2017
 * callback
 */
include('config.php');
$content="";
...
$db->query("INSERT INTO apps_reverse_response (api_id, reverse_response_content, response_time) VALUES ('2', '".$content."', NOW())");

if(isset($_GET['rid']) && $_GET['rid']!='' && isset($_GET['transtype']) && $_GET['transtype']!='' && isset($_GET['rchNo']) && $_GET['rchNo']!='' && isset($_GET['amt']) && $_GET['amt']!='') {
	
	$recharge_id = trim(mysql_real_escape_string($_GET['rid']));
	$api_status = trim(mysql_real_escape_string($_GET['transtype']));
	$amount = trim(mysql_real_escape_string($_GET['amt']));
	$account = trim(mysql_real_escape_string($_GET['rchNo']));
	$api_txn_no = trim(mysql_real_escape_string($_GET['tid']));
	
	$recharge_info=$db->queryUniqueObject("SELECT * FROM apps_recharge WHERE account_no='".$account."' AND amount='".$amount."' AND recharge_id='".$recharge_id."' AND api_id='2' ");
	if($recharge_info) {
		//operator row
		...
		if($agent_info) {
			...
			if($api_status=='SUCCESS') {
				...
				$operator_ref_no=trim(mysql_real_escape_string($_GET['txid']));
