...
$sWhere = "WHERE user.user_type = '5' AND mdist_id = '".$_SESSION['mdistributor_uid']."' ";
...
$sArray = "WHERE user_type = '4' AND mdist_id = '".$_SESSION['mdistributor_uid']."' ";
if(isset($_GET['status']) && $_GET['status'] != '') {
...
$arUser1 = array();
$qry1 = $db->query("SELECT * FROM apps_user $sArray ORDER BY company_name ASC ");
while($rst1 = $db->fetchNextObject($qry1)) {
	$arUser1[] = array('uid'=>$rst1->uid, 'name'=>$rst1->company_name);
}

$arUser = array();
$qry = $db->query("SELECT * FROM apps_user WHERE user_type = '4' AND mdist_id = '".$_SESSION['mdistributor_uid']."' ");
				...
				<table class="table table-condensed table-striped table-bordered">
					...
					<tbody>
						...
						<tr>
							...
							<td><?php echo distName($arUser, $result->dist_id);?></td>
							</td>
							<td align="center">
