			...
			<div class="box-body no-padding">
				<table class="table table-striped">
					...
					<tbody>
						<?php					
						$scnt = 1;
						$query = $db->query("SELECT opr.operator_name, opr.service_type, ser.service_name, com.comm_mdist, com.comm_dist, com.comm_ret, com.is_surcharge, com.is_percentage, com.surcharge_value FROM operators opr LEFT JOIN service_type ser ON opr.service_type = ser.service_type_id LEFT JOIN usercommissions com ON opr.operator_id = com.operator_id AND com.uid = '".$_SESSION['mdistributor_uid']."' ORDER BY opr.service_type, opr.operator_name ASC");
						while($row = $db->fetchNextObject($query)) { ?>
						<tr>
							<td align="center"><?php echo $scnt++;?></td>
							<td><?php echo $row->operator_name;?></td>
							<td><?php echo $row->service_name;?></td>
							<td><?php echo $row->comm_mdist;?></td>
