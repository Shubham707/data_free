 ...
 * callback {status} {mobile} {amount} {tranid} {agentid} {liveid} {msg}
 */
include('config.php');
$content="";
...
$db->query("INSERT INTO apps_reverse_response (api_id, reverse_response_content, response_time) VALUES ('7', '".$content."', NOW())");

if(isset($_GET['status']) && $_GET['status']!='' && isset($_GET['mobile']) && $_GET['mobile']!='' && isset($_GET['amount']) && $_GET['amount']!='') {
	...
	if(isset($_GET['agentid']) && $_GET['agentid']!='') {
		...
		$recharge_id = trim(mysql_real_escape_string($_GET['agent']));
	}
	if(isset($_GET['tranid']) && $_GET['tranid']!='') {
		$api_txn_no = trim(mysql_real_escape_string($_GET['tranid']));
	} else {
	...
	if($recharge_info) {
		...
		if($agent_info) {
			...
			if($api_status=='SUCCESS') {
				...
				if(isset($_GET['liveid']) && $_GET['liveid']!='') {
					$operator_ref_no = trim(mysql_real_escape_string($_GET['liveid']));
				} else {
				...
				*/
				...
				if($recharge_info->is_callback=='0') {
					...
					if($sCommission['rtCom']>'0') {
						...
						$db->execute("INSERT INTO `commission_details`(`recharge_id`, `uid`, `amount`, `closing_balance`, `added_date`) VALUES ('".$recharge_id."', '".$agent_info->uid."', '".$sCommission['rtCom']."', '".$rt->closing_balance."', NOW())");
					}
					...
					if($sCommission['dsCom']>'0') {
						...
						if($ds) {										
							...
							$db->execute("INSERT INTO `commission_details`(`recharge_id`, `uid`, `amount`, `closing_balance`, `added_date`) VALUES ('".$recharge_id."', '".$agent_info->dist_id."', '".$sCommission['dsCom']."', '".$ds_close_balance."', NOW())");
							if($ts1) {
				...
				*/
				...
				if($recharge_info->is_callback=='0') {
					 ...
					 */
					$resp_time=date("Y-m-d H:i:s", strtotime('-10 minutes'));
			...
			if($agent_info->user_type=='1') {
				...
				if($setting_info->reverse_url!='') {
					...
					curl_close($ch);				
					$db->execute("INSERT INTO `reverse_url_log`(`client_id`, `api_id`, `url_detail`, `status_code`, added_date) VALUES ('".$agent_info->uid."', '".$recharge_info->api_id."', '".$hitUrl."', '".$http_code."', NOW())");
