...
if($agent_info) {	
	$isMasterMoney = $db->queryUniqueObject("SELECT is_money FROM apps_user WHERE uid='".$agent_info->mdist_id."' AND is_money='a' ");
	$isDistMoney = $db->queryUniqueObject("SELECT is_money FROM apps_user WHERE uid='".$agent_info->dist_id."' AND is_money='a' ");
	if($isMasterMoney && $isDistMoney) {
				...
				if($operator_info->api_status=='1') {
					if($operator_info->status=='1') {
						if($agent_info->user_type=='1') {				
							...
							$sCommission = getUsersCommissionNew(trim($agent_info->mdist_id), $operator_info->operator_id, $amount, 'r');
						}
						...
						if($isfund && (($isfund->balance - $isfund->cuttoff) >= ($amount + $sCommission['samount']))) {
							...
							if($wallet && $ts1) {
								...
								if($commit) {
									...
									if($api_id=='11') {
										$status = '0';
										$status_details = 'Transaction successfull';
										$api_txn_no = '';
										$api_status = '';
										$api_status_details = '';
										$operator_ref_no = '';
										$jsDmtResponse = '{"EZY_RSP":"0","EZY_MSG":"Transaction successfull","EZY_MOB":"'.$account.'","EZY_BEN_NAME":"","EZY_BEN_CODE":"","EZY_BANK_NAME":"","EZY_BEN_IMPS_REF_NO":"","EZY_TRANS_ID":"","EZY_MESSAGE":""}';
									} else {							
										...
										*/
										...
										if($agent_info->user_type!='1') {								
											...
											if($sCommission['mdCom'] > '0') {
												$db->query("START TRANSACTION");
												$mDs = $db->queryUniqueObject("SELECT wallet_id, balance FROM apps_wallet WHERE uid='".$agent_info->mdist_id."' ");
												if($mDs) {										
													$md_close_balance = $mDs->balance + $sCommission['mdCom'];
													$db->execute("UPDATE apps_wallet SET balance='".$md_close_balance."' WHERE wallet_id='".$mDs->wallet_id."'");
													$ts2 = mysql_affected_rows();
									...
									elseif($status=='2') {
										...
										if($wallet1 && $ts4) {													
											$commit = $db->query("COMMIT");	
											if($commit) {													
												$remark_new = "DMT REVERT: $recharge_id, $account, $amount, $debit_amount, failed revert";
						...
						$jsDmtResponse = '{"EZY_RSP":"29","EZY_MSG":"Operator Downtime, Try after sometime"}';	
					}
				} else {
					$jsDmtResponse = '{"EZY_RSP":"30","EZY_MSG":"Operator API is Down"}';	
				}
			} else {
				$jsDmtResponse = '{"EZY_RSP":"31","EZY_MSG":"Operator Service is Down"}';	
			}		
		} else {
			$jsDmtResponse = '{"EZY_RSP":"23","EZY_MSG":"Invalid DMT operation"}';	
		}
	} else {
		$jsDmtResponse = '{"EZY_RSP":"23","EZY_MSG":"IMPS Server Request Timeout"}';	
	}
} else {
