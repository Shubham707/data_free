...
$countMobileChange = $db->countOf("mobile_change_request", "request_status='0' ");
$countDmt = $db->countOf("dmt_activation_request", "dmt_request_status='0' ");
...
<div class="content">
	<div class="container-fluid">
		...
		<?php } ?>
		<?php if($countMobileChange > 0) { ?>
		<div class="alert alert-red">
			You have <span class="badge"><?php echo $countMobileChange;?></span> mobile number chnage request is pending, to view click to button <a href="mobile-change-request.php" class="btn btn-sm btn-info">List Request</a>
		</div>
		<?php } ?>
		<?php if($countDmt > 0) { ?>
		<div class="alert alert-red">
			You have <span class="badge"><?php echo $countDmt;?></span> Money Transfer activations request is pending, to view click to button <a href="dmt-activation-request.php" class="btn btn-sm btn-info">List Request</a>
		</div>
		<?php } ?>
		<div class="row">
			<div class="col-sm-8">
				<div class="row">
					...
					<div class="col-sm-3">
						<div class="small-box">
							<a href="admin-dashboard.php" class="small-block">
					...
					<div class="col-sm-3">
						<div class="small-box">
							<a href="rpt-sale-ret-form.php" class="small-block">
								<i class="fa fa-table"></i>
								<p>Monthly Retailer Sale Report</p>
							</a>
						</div>
					</div>
				</div>
			...
			<div class="col-sm-4">
				...
				<div class="box">
					...
					<div class="box-body max-height-300 overflow-auto">
						<?php
						$query = $db->query("SELECT * FROM activity_login WHERE user_type!='0' AND (login_time > DATE_SUB(NOW(), INTERVAL 1 HOUR)) GROUP BY username ");
