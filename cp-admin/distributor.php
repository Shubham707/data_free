...
if(isset($_GET['mdist_id']) && $_GET['mdist_id'] != '') {
	$sWhere .= " AND user.mdist_id='".mysql_real_escape_string($_GET['mdist_id'])."' ";
}
...
if(isset($_GET['status']) && $_GET['status']!='') {
	...
	$sArray = " AND status!='9' AND status!='0' ";
} 
$arUser1 = array();
$qry1 = $db->query("SELECT * FROM apps_user WHERE user_type='3' AND status='1' $sArray ");
while($rst1 = $db->fetchNextObject($qry1)) {
	$arUser1[] = array('id'=>$rst1->uid, 'name'=>$rst1->company_name);
}

$arUser = array();
$qry = $db->query("SELECT * FROM apps_user WHERE 1 ");
			...
			<div class="box-body no-padding min-height-480">
				<div class="box-filter padding-20">
					<form method="get">
						...
						<div class="col-sm-4">
							<div class="form-group">
								<select name="mdist_id" class="form-control">
									<option value="">---Master Distributor---</option>
									<?php foreach($arUser1 as $key=>$data) { ?>
									<option value="<?php echo $data['id'];?>" <?php if(isset($_GET['mdist_id']) && $_GET['mdist_id'] == $data['id']) { ?> selected="selected"<?php } ?>><?php echo $data['name'];?></option>
									<?php } ?>
								</select>
							</div>
						</div>
				...
				<table class="table table-condensed table-striped table-bordered">
					...
					<tbody>
						...
						<tr>
							...
							<td><?php echo ucwords($result->user_category);?></td>
							...
							<td align="center"><a class="fancyToken fancybox.ajax btn btn-xs btn-inverse" href="token-add.php?uid=<?php echo $result->uid;?>"><?php echo $result->activation_token;?></a></td>
							<td>
							    <?php
							    	$sql="select sum(amount) as sale_amount from transactions where account_id=".$result->uid." and transaction_date like'%".date('Y-m-d')."%' and transaction_term='FUND' and type='dr'";
							...
							<td><?php echo mDistName($arUser, $result->mdist_id);?></td>
							<td><?php
						...
						<td style="text-align:center;">
							...
							<td style="text-align:center;">
								<a href="user-whitelabel.php?id=<?php echo $result->user_id;?>" title="details" class="btn btn-xs btn-info"><i class="fa fa-cog"></i></a>
							</td>
							<td align="center">
