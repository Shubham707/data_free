				...
				<table class="table table-condensed table-basic">
					...
					<tbody>
						...
						<tr>
							...
							<?php if($row->type == 'dr') { ?>
							<td align="right"><?php echo round($row->amount_in,2);?></td>
							<td align="right"><?php echo round($row->amount_out,2);?></td>
					...
					<tfoot>
						<?php $qry = $db->query("SELECT trans.*, SUM(IF(trans.type = 'dr', trans.amount, 0)) AS debitAmount, SUM(IF(trans.type = 'cr', trans.amount, 0)) AS creditAmount, SUM(trans.amount_in) AS AmountIn, SUM(trans.amount_out) AS AmountOut FROM {$statement}");
						$row = $db->fetchNextObject($qry); ?>
						<tr>
							<td align="right" colspan="7"><b>Total</b></td>
							<td align="right"><b class="text-primary"><?php echo round($row->AmountIn,2);?></b></td>
							<td align="right"><b class="text-primary"><?php echo round($row->AmountOut,2);?></b></td>
