...
ob_start();
$data[] = array('S.No', 'Txn Date','UID', 'User Details','ACC Mnger', 'Type','Opening Balance', 'Debit Amount', 'Credit Amount', 'Closing Balance','Calculation','Difference', 'Term', 'Ref Txn No', 'Remark', 'User Type', 'Transaction User');
...
$scnt = 1;
$query = $db->query("SELECT trans.*, user.company_name,user.uid, adm.fullname FROM {$statement}");
...
while($result = $db->fetchNextObject($query)) {
	...
	if($result->type == 'dr') {
		...
		$cal=$opening_balance-$result->amount;
		$credit_amount = "";
	} else {
	...
	$data[] = array($scnt++, $result->transaction_date,$result->uid,  $result->company_name,  $result->fullname, $result->type,$opening_balance, $debit_amount, $credit_amount, $result->closing_balance,$cal,$diff , $result->transaction_term, $result->transaction_ref_no, $result->remark, $result->transaction_user_type, $result->transaction_by);
