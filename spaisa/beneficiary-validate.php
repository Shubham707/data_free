...
if($agent_info) {
	...
	if($operator_info) {
		...
		if($wallet && (($wallet->balance - $wallet->cuttoff) >= $debit_amount)) {
			 ...
			 */
			include(DIR."/spaisa/beneficiary-validate-api.php");
			...
			if($status == '0') {
				...
				*/
				if($sCommission['rtCom'] > '0') {
					$db->execute("INSERT INTO `user_commission_info`(`user_com_id`, `recharge_id`, `uid`, `amount`, `closing_balance`, `added_date`) VALUES ('', '".$recharge_id."', '".$agent_info->uid."', '".$sCommission['rtCom']."', '".$close_balance."', NOW())");
				}
				...
				*/												
				...
				$remark_new = "DMT VALIDATE: $recharge_id, $account, $amount, $debit_amount, Failed Revert";				
			 ...
			 */
			if(isset($output)) {
				$db->execute("INSERT INTO `recharge_log`(`recharge_log_id`, `txn_no`, `api_id`, `http_response_code`, `http_response_content`, `update_date`) VALUES ('', '".$recharge_id."', '".$api_id."', '".$http_code."', '".$output."', NOW())");
			}
