...
$db->query("INSERT INTO apps_reverse_response (api_id, reverse_response_content, response_time) VALUES ('14', '".$content."', NOW())");

if(isset($_POST['status']) && $_POST['status']!='' && isset($_POST['transaction_id']) && $_POST['transaction_id']!='' && isset($_POST['operator_id']) && $_POST['operator_id']!='') {
	
	$api_status = trim(mysql_real_escape_string($_POST['status']));
	$operator_ref_no = trim(mysql_real_escape_string($_POST['operator_id']));
	$api_txn_no = trim(mysql_real_escape_string($_POST['transaction_id']));
	
	$recharge_info=$db->queryUniqueObject("SELECT * FROM apps_recharge WHERE api_txn_no='".$api_txn_no."' AND api_id='14' ");
	if($recharge_info) {
		//operator row
