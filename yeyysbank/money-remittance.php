		...
		if($operator_info->ser_status=='1') {
			if($operator_info->api_status=='1') {
				if($operator_info->status=='1') {
						...
						if($wallet && $ts1) {
							...
							if($commit) {
								...
								if($api_id=='11') {
									...
									$api_status_details = '';
									$operator_ref_no = '';
									$jsDmtResponse = '{"RESP_CODE":"300","RESP_MSG":"Trasaction Successfull.","RESP_MOB":"'.$account.'"}';
								} else {							
									include(DIR."/yesbank/money-remittance-api.php");
								}
					...
					$jsDmtResponse = '{"ResponseCode":"29","Message":"Operator Downtime, Try after sometime"}';	
				}
			} else {
				$jsDmtResponse = '{"ResponseCode":"30","Message":"Operator API is Down"}';	
			}
		} else {
			$jsDmtResponse = '{"ResponseCode":"31","Message":"Operator Service is Down"}';	
		}		
	} else {
