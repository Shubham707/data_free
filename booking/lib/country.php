...
if(@$_REQUEST['country'])
{
	...
	echo ' <label for="name" class="">State:</label><select  name="state" id="state22" class="form-control" onclick="selectTours(this.value)">';
...
if(@$_REQUEST['state'])
{
	...
	$query=mysqli_query($db,$sql) or die('query Not Execute!');
	echo '<label for="name" class="">City:</label> <select  name="city" id="city" class="form-control">';
