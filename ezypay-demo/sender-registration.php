...
$request_txn_no = time();
$ezpmsg = "msg=E06003~".$ezykey."~".$request_txn_no."~".$account."~".$name."~NA~NA";
$url = 'http://180.179.20.116:8030/RemitMoney/mtransfer';
if($account=='9826386456') {
	$output = "E06004~".$ezykey."~".$account."~".$name."~123456~SUCCESS~Description~NA~NA";
} else {
	$output = "E06004~".$ezykey."~".$account."~".$name."~NA~FAILED~Sender not registered~NA~NA";
}
if($output) {	
	...
	if($ezrsp[5]=='SUCCESS') {
		$jsDmtResponse = '{"EZY_RSP":"0","EZY_MSG":"'.$ezrsp[6].'","EZY_MOB":"'.$account.'","EZY_REF_TXN_NO":"'.$request_txn_no.'","EZY_VERIFICATION_REF_NO":"'.$ezrsp[4].'"}';	
